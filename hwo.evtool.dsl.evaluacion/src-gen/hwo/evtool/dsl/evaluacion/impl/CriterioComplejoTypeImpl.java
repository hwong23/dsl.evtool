/**
 * generated by Xtext 2.25.0
 */
package hwo.evtool.dsl.evaluacion.impl;

import hwo.evtool.dsl.evaluacion.CmpntEvaluacion;
import hwo.evtool.dsl.evaluacion.CriterioComplejoType;
import hwo.evtool.dsl.evaluacion.EvaluacionPackage;

import org.eclipse.emf.common.notify.Notification;

import org.eclipse.emf.ecore.EClass;
import org.eclipse.emf.ecore.InternalEObject;

import org.eclipse.emf.ecore.impl.ENotificationImpl;

/**
 * <!-- begin-user-doc -->
 * An implementation of the model object '<em><b>Criterio Complejo Type</b></em>'.
 * <!-- end-user-doc -->
 * <p>
 * The following features are implemented:
 * </p>
 * <ul>
 *   <li>{@link hwo.evtool.dsl.evaluacion.impl.CriterioComplejoTypeImpl#getComponente <em>Componente</em>}</li>
 * </ul>
 *
 * @generated
 */
public class CriterioComplejoTypeImpl extends CriterioImpl implements CriterioComplejoType
{
  /**
   * The cached value of the '{@link #getComponente() <em>Componente</em>}' reference.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @see #getComponente()
   * @generated
   * @ordered
   */
  protected CmpntEvaluacion componente;

  /**
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  protected CriterioComplejoTypeImpl()
  {
    super();
  }

  /**
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  @Override
  protected EClass eStaticClass()
  {
    return EvaluacionPackage.Literals.CRITERIO_COMPLEJO_TYPE;
  }

  /**
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  @Override
  public CmpntEvaluacion getComponente()
  {
    if (componente != null && componente.eIsProxy())
    {
      InternalEObject oldComponente = (InternalEObject)componente;
      componente = (CmpntEvaluacion)eResolveProxy(oldComponente);
      if (componente != oldComponente)
      {
        if (eNotificationRequired())
          eNotify(new ENotificationImpl(this, Notification.RESOLVE, EvaluacionPackage.CRITERIO_COMPLEJO_TYPE__COMPONENTE, oldComponente, componente));
      }
    }
    return componente;
  }

  /**
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  public CmpntEvaluacion basicGetComponente()
  {
    return componente;
  }

  /**
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  @Override
  public void setComponente(CmpntEvaluacion newComponente)
  {
    CmpntEvaluacion oldComponente = componente;
    componente = newComponente;
    if (eNotificationRequired())
      eNotify(new ENotificationImpl(this, Notification.SET, EvaluacionPackage.CRITERIO_COMPLEJO_TYPE__COMPONENTE, oldComponente, componente));
  }

  /**
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  @Override
  public Object eGet(int featureID, boolean resolve, boolean coreType)
  {
    switch (featureID)
    {
      case EvaluacionPackage.CRITERIO_COMPLEJO_TYPE__COMPONENTE:
        if (resolve) return getComponente();
        return basicGetComponente();
    }
    return super.eGet(featureID, resolve, coreType);
  }

  /**
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  @Override
  public void eSet(int featureID, Object newValue)
  {
    switch (featureID)
    {
      case EvaluacionPackage.CRITERIO_COMPLEJO_TYPE__COMPONENTE:
        setComponente((CmpntEvaluacion)newValue);
        return;
    }
    super.eSet(featureID, newValue);
  }

  /**
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  @Override
  public void eUnset(int featureID)
  {
    switch (featureID)
    {
      case EvaluacionPackage.CRITERIO_COMPLEJO_TYPE__COMPONENTE:
        setComponente((CmpntEvaluacion)null);
        return;
    }
    super.eUnset(featureID);
  }

  /**
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  @Override
  public boolean eIsSet(int featureID)
  {
    switch (featureID)
    {
      case EvaluacionPackage.CRITERIO_COMPLEJO_TYPE__COMPONENTE:
        return componente != null;
    }
    return super.eIsSet(featureID);
  }

} //CriterioComplejoTypeImpl
